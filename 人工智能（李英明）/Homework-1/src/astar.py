        ...
        while not open_set.empty():
            ...
            for new_state, move_cost in new_states:
                ...
                if is_goal_state:
                    ...
                    open_set.push(new_state.board, new_cost + heuristic_func2(new_state, goal_state))
                    # open_set优先队列用f值进行排序，即从起点到该状态的cost加上从该状态到终点的cost
                    # 用每个状态的f值标记优先级，确保每次pop出来的都是f值最小的节点
                    # 如果新状态未访问过，则会加入到open_set当中；如果访问过则会更新对应的优先级(即f值)
                    """
                    TODO 9:
...
if __name__ == "__main__":
    ...
    goal_state = State(((7, 3, 1),
                        (8, 0, 4),
                        (6, 5, 2)))
    solver = AStarSearch(init_state, goal_state, move_type=2)
    solver.solve()
    solver.print()
